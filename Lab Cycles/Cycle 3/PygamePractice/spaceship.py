...
BORDER = (Wid/2-5,0,10,Hei)
#Declaring the main surface
def draw_window(red,yellow):
    WIN.fill((COLOUR)) 
    pygame.draw.rect(WIN,COLOUR2,BORDER)
...
def yellow_handle_movement(keys_pressed,yellow):
        if keys_pressed[pygame.K_a] and yellow.x - VEL > 0: #LEFT
            yellow.x-=VEL
        if keys_pressed[pygame.K_d] and yellow.x + VEL + yellow.width< BORDER[0]: #RIGHT
            yellow.x+=VEL
        if keys_pressed[pygame.K_w] and yellow.y - VEL > 0: #UP
            yellow.y-=VEL
        if keys_pressed[pygame.K_s] and yellow.y + VEL + yellow.height < Hei - 20: #DOWN
            yellow.y+=VEL
